...
'''
PEPPER_META_XML = '''<?xml version="1.0"?>
<paula version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink">
<header paula_id="demo.meta_author"/>
<featList type="author" xml:base="meta"><feat xlink:href="#anno_1" value="Demo Author"/></featList>
</paula>
'''
...
class PaulaAuditContractTests(unittest.TestCase):
    ...
    def test_inventories_paula_xml_kinds_and_metadata_features(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self._write_archive(root, "demo", "demo", {
        ...
        self.assertEqual(report["element_kind_counts"], {"body": 1, "featList": 2, "markList": 1, "relList": 1})
        self.assertEqual(report["list_type_occurrences"], {"featList": {"license": 1, "pos": 1}, "markList": {"tok": 1}, "relList": {"dep": 1}})
        self.assertEqual(report["metadata_feature_type_occurrences"], {"license": 1})
        self.assertEqual(report["metadata_feature_instances"], [{"dataset": "demo/demo", "source": "demo/demo_PAULA.zip!/demo/demo.meta_license.xml", "paula_id": "demo.meta_license", "base": "demo.anno.xml", "type": "license", "values": ["CC-BY 4.0"]}])
        self.assertEqual(report["errors"], [])

    def test_pepper_meta_base_is_metadata(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self._write_archive(root, "demo", "demo", {"demo/demo.meta_author.xml": PEPPER_META_XML})
            report = self.audit.audit_upstream(root)

        self.assertEqual(report["metadata_feature_type_occurrences"], {"author": 1})
        self.assertEqual(report["metadata_feature_instances"][0]["base"], "meta")
        self.assertEqual(report["metadata_feature_instances"][0]["values"], ["Demo Author"])
    ...
    def test_annofeat_is_not_metadata_and_multifeat_metadata_is_expanded(self):
        ...
        self.assertEqual(report["list_type_occurrences"], {"featList": {"annoFeat": 1}, "multiFeatList": {"multiFeat": 1}})
        self.assertEqual(report["metadata_feature_type_occurrences"], {"language": 1, "source_format": 1})
        self.assertEqual(report["metadata_feature_instances"], [
            {"dataset": "demo/demo", "source": "demo/demo_PAULA.zip!/demo/demo.meta_multiFeat.xml", "paula_id": "demo.meta_multiFeat", "base": "demo.anno.xml", "type": "language", "values": ["Coptic"]},
            {"dataset": "demo/demo", "source": "demo/demo_PAULA.zip!/demo/demo.meta_multiFeat.xml", "paula_id": "demo.meta_multiFeat", "base": "demo.anno.xml", "type": "source_format", "values": ["PAULA XML"]},
        ])

    def test_nested_archive_packaging_is_counted_once(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self._write_archive(root, "nested", "nested", {"nested/doc.text.xml": TEXT_XML}, nested=True)
            report = self.audit.audit_upstream(root)
        self.assertEqual(report["source_package_count"], 1)
        self.assertEqual(report["parsed_package_count"], 1)
    ...
    def test_malformed_xml_is_reported_with_member_identity(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self._write_archive(root, "broken", "broken", {"broken/good.text.xml": TEXT_XML, "broken/bad.xml": "<paula><broken>"})
            report = self.audit.audit_upstream(root)
        self.assertEqual(report["source_package_count"], 1)
        self.assertEqual(report["parsed_package_count"], 1)
        self.assertEqual(len(report["errors"]), 1)
        self.assertEqual(report["errors"][0]["kind"], "malformed_xml")
        self.assertEqual(report["errors"][0]["source"], "broken/broken_PAULA.zip!/broken/bad.xml")

    def test_report_is_deterministic(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self._write_archive(root, "demo", "demo", {"z/doc.pos.xml": POS_XML, "a/demo.meta_license.xml": META_XML, "m/doc.text.xml": TEXT_XML})
