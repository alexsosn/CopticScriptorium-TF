...
COPTIC_META_XML = '<paula version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink"><header paula_id="doc.license"/><featList type="license" xml:base="anno.xml"><feat xlink:href="#anno_1" value="CC-BY 4.0"/></featList></paula>'
...
class PaulaAuditContractTests(unittest.TestCase):
    ...
    def test_metadata_and_feature_examples(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self._write_archive(
                ...
                "demo",
                {
                    "doc.text.xml": TEXT_XML,
                    "doc.pos.xml": POS_XML,
                    "demo.meta_license.xml": META_XML,
                    "demo.meta_author.xml": PEPPER_META_XML,
                },
            )
    ...
    def test_coptic_anno_xml_base_is_document_metadata(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self._write_archive(root, "demo", "demo", {"doc.license.xml": COPTIC_META_XML})
            report = self.audit.audit_upstream(root)
        self.assertEqual(report["metadata_feature_type_occurrences"], {"license": 1})
        self.assertEqual(report["metadata_feature_instances"][0]["base"], "anno.xml")
        self.assertEqual(report["metadata_feature_instances"][0]["values"], ["CC-BY 4.0"])

    def test_outer_wrapper_zip_with_inner_paula_zip_is_parsed(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            inner = BytesIO()
            with zipfile.ZipFile(inner, "w") as archive:
                archive.writestr("bohairic.nt/doc.license.xml", COPTIC_META_XML)
                archive.writestr("bohairic.nt/doc.text.xml", TEXT_XML)
            self._write_archive(
                root,
                "bohairic.nt",
                "bohairic.nt",
                {"bohairic.nt_PAULA.zip": inner.getvalue()},
            )
            report = self.audit.audit_upstream(root)
        self.assertEqual(report["source_package_count"], 1)
        self.assertEqual(report["parsed_package_count"], 1)
        self.assertEqual(report["xml_member_count"], 2)
        self.assertEqual(report["metadata_feature_type_occurrences"], {"license": 1})
        self.assertEqual(report["packages_without_xml_members"], [])
    ...
    def test_annofeat_is_not_metadata_and_multifeat_metadata_is_expanded(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self._write_archive(
                ...
                "demo",
                {
                    "demo.anno_feat.xml": ANNOFEAT_XML,
                    "demo.meta_multiFeat.xml": MULTI_META_XML,
                },
            )
            report = self.audit.audit_upstream(root)
    ...
    def test_no_xml_package_records_member_names(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self._write_archive(root, "opaque", "opaque", {"README.txt": "x", "config.json": "{}"})
            report = self.audit.audit_upstream(root)
        self.assertEqual(
            report["packages_without_xml_members"],
            [{"dataset": "opaque/opaque", "source": "opaque/opaque_PAULA.zip", "members": ["README.txt", "config.json"]}],
        )
    ...
    def test_malformed_xml_is_reported(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self._write_archive(root, "broken", "broken", {"good.text.xml": TEXT_XML, "bad.xml": "<paula><broken>"})
            report = self.audit.audit_upstream(root)
    ...
    def test_report_is_deterministic(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self._write_archive(root, "demo", "demo", {"z.pos.xml": POS_XML, "a.meta.xml": META_XML, "m.text.xml": TEXT_XML})
